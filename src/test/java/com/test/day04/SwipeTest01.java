...
//        }
        while(true){
            swipeLeft(300);
            //等待“立即体验”可见跳出循环
            /*WebDriverWait webDriverWait = new WebDriverWait(androidDriver,2);
            WebElement webElement =  webDriverWait.until(ExpectedConditions.
                    visibilityOfElementLocated(MobileBy.id("com.xxzb.fenwoo:id/btn_start")));
            if*/
            //根据pageSource是否包含此元素来做判断
            if(androidDriver.getPageSource().contains("com.xxzb.fenwoo:id/btn_start")){
                break;
            }
        }
        androidDriver.findElement(MobileBy.id("com.xxzb.fenwoo:id/btn_start")).click();
    }
